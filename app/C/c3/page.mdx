...
Tablica to **ciąg elementów tego samego typu ułożonych w pamięci kolejno, bez
przerw**. Elementy tablicy mają:

- **wspólną nazwę**,
- **ten sam typ**,
- są dostępne przez **indeks**, zaczynający się od **0**.

Tablice są fundamentem pracy w C — są szybkie, przewidywalne i bardzo
niskopoziomowe.
...
> [!NOTE]
>
> **Standard C nie sprawdza indeksów tablicy.** `tab[9999] = 7;` _skompiluje
> się_, ale prowadzi do nadpisania pamięci → błąd krytyczny.
...
> [!NOTE]
>
> `sizeof(t)` → liczba **bajtów całej tablicy** `sizeof(t[0])` → liczba **bajtów
> pojedynczego elementu**
>
> Dzieląc te wartości dostajemy **liczbę elementów tablicy**, niezależnie od
> typu i rozmiaru elementu.
>
> To działa jedynie dla **tablic statycznych (lokalnych/globalnych)**. Dla
> wskaźników nie działa — o tym w dziale o wskaźnikach.
...
Tablica 2D jest fizycznie **jednym dużym blokiem pamięci**. C nie tworzy tablic
tablic — tworzy **ciąg liczb**, ale interpretuje go jak macierz.
...
Ale **nie rób tego na początku nauki** — to przyda się w optymalizacji i
...
W C **nie ma typu `string`**. Napis to **ciąg znaków zakończony
null-terminatorem `'\0'`**.
...
> ```
>
> automatycznie dodaje **`'\0'` na końcu**. To zachowanie jest gwarantowane
> przez standard C.
...
> [!NOTE]
>
> `fgets()` wczytuje max `sizeof(buf) - 1` znaków i **zawsze** terminator
> `'\0'`.
...
<QuizQuestion
    ...
    options={[
        ...
        {
            id: 'b',
            text: 'Tablica jest ciągłym blokiem pamięci',
            correct: true,
        },
        { id: 'c', text: 'Tablice mają dynamiczny rozmiar', correct: false },
        {
            id: 'd',
            text: 'C sprawdza poprawność indeksów tablicy',
            correct: false,
        },
    ]}
/>
...
<QuizQuestion
    ...
    options={[
        { id: 'a', text: '12', correct: false },
        {
            ...
            text: '13',
            correct: true,
            explanation: 'Musi zmieścić 12 znaków + terminator \\0.',
        },
